...
export default function RoutinesClient({ routines }: { routines: RoutineWithCount[] }) {
    ...
    async function startEmptyWorkout() {
        ...
        const { data: session } = await supabase
            ...
            .insert({ user_id: user.id, title: 'Empty Workout', status: 'active' })
            .select()
            .single()
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
                ...
                {routines.map(routine => (
                    <div
                        key={routine.id}
                        className="glass"
                        style={{ padding: 'var(--spacing-md)', position: 'relative', cursor: 'pointer' }}
                    >
                        {/* 3-Punkte Menü */}
                        <button
                        ...
                        <div onClick={() => startRoutine(routine)}>
                            ...
                            <p style={{
                                fontSize: 'var(--font-size-sm)',
                                color: 'var(--color-text-secondary)',
                                marginBottom: routine.description ? 'var(--spacing-xs)' : 0,
                            }}>
