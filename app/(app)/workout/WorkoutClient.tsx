  ...
  return (
    <div style={{ position:'relative', overflow:'hidden' }}>
      ...
      <div ref={rowRef} style={{
        ...
        <input type="number" inputMode="decimal" placeholder="0" value={s.weight_kg??''} onChange={e=>onUpdate('weight_kg',e.target.value)} disabled={isCompleted}
          style={{ textAlign:'center', width:'100%', fontSize:'var(--font-size-base)', fontWeight:'500', opacity:isCompleted?0.5:1 }} />
        <input type="number" inputMode="numeric" placeholder="0" value={s.reps??''} onChange={e=>onUpdate('reps',e.target.value)} disabled={isCompleted}
          style={{ textAlign:'center', width:'100%', fontSize:'var(--font-size-base)', fontWeight:'500', opacity:isCompleted?0.5:1 }} />
...
export default function WorkoutClient({ session, initialExercises, initialSets, existingPRs, previousSets }: Props) {
  ...
  useEffect(()=>{ if(initialExercises.length===0) return; loadEDBDataForExercises(initialExercises).then(setExercises) },[initialExercises.length])
  ...
  async function finishWorkout(){
    const pi:any[]=[]
    for(const[sid,pt] of Object.entries(newPRs)){ if(!pt.length) continue; const s=sets.find(x=>x.id===sid); if(!s) continue; const ex=exercises.find(e=>e.id===s.workout_exercise_id); if(!ex) continue
        ...
        {editingTitle ? (
          <input
            ...
            style={{
              ...
              border: 'none',
              borderBottom: '1px solid color-mix(in srgb, var(--color-primary) 60%, transparent)',
          ...
          <button
            ...
            style={{
              ...
              color: 'var(--color-text-primary)',
              fontWeight: '600', fontSize: 'var(--font-size-md)',
              cursor: 'text', padding: '4px 8px',
              borderRadius: '6px',
