...
interface Props {
  session: WorkoutSession; initialExercises: WorkoutExercise[]
  initialSets: WorkoutSet[]; existingPRs: { exercise_id: string; pr_type: string; value: number }[]
...
function SwipeableSetRow({ s, i, isCompleted, hasPR, isHighlighted, prevValue, onComplete, onUpdate, onDelete }: {
  ...
  prevValue: string
  onComplete: () => void; onUpdate: (f: string, v: string) => void; onDelete: () => void
}) {
  ...
  useEffect(() => {
    ...
    function apply(val: number, anim = false) {
      off = val
      row.style.transition = anim ? 'transform 0.25s ease' : 'none'
      row.style.transform = `translateX(${val}px)`
      if (bgRef.current) bgRef.current.style.opacity = String(Math.min(1, Math.abs(val) / 60))
    }
    ...
    function tm(e: TouchEvent) {
      ...
      if (dir === 'v') return
      if (dx > 0) { apply(0, true); return }
      e.preventDefault(); apply(Math.max(dx, -100))
    }
    function te() {
      if (dir === 'h' && off < -60) { apply(-110, true); setTimeout(() => cbRef.current(), 220) }
      else apply(0, true)
    }
    row.addEventListener('touchstart', ts, { passive: true })
    row.addEventListener('touchmove', tm, { passive: false })
    row.addEventListener('touchend', te, { passive: true })
  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden' }}>
      <div ref={bgRef} style={{ position: 'absolute', inset: 0, opacity: 0, background: 'rgba(255,90,90,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: '20px', pointerEvents: 'none' }}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FF5A5A" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div ref={rowRef} style={{
        ...
        background: isHighlighted ? 'color-mix(in srgb, var(--color-primary) 18%, transparent)'
          : isCompleted ? 'color-mix(in srgb, var(--color-success) 15%, transparent)'
        ...
        outline: isHighlighted ? '1px solid color-mix(in srgb, var(--color-primary) 50%, transparent)' : 'none',
      }}>
        <span style={{ textAlign: 'center', fontWeight: '600', fontSize: 'var(--font-size-sm)', color: isCompleted ? 'var(--color-success)' : 'var(--color-text-secondary)' }}>
          {hasPR ? '🥇' : s.set_number}
        </span>
        ...
        <button onClick={onComplete} style={{
          ...
          border: isCompleted ? '2px solid var(--color-success)' : '2px solid rgba(255,255,255,0.2)',
          background: isCompleted ? 'color-mix(in srgb, var(--color-success) 30%, transparent)' : 'transparent',
          color: isCompleted ? 'var(--color-success)' : 'transparent',
...
// ─── Exercise Menu ─────────────────────────────────────────────────────────────
function ExerciseMenu({ isInSuperset, canPair, onStartPairing, onDissolve, onRemove, onClose }: {
  isInSuperset: boolean; canPair: boolean
  onStartPairing: () => void; onDissolve: () => void; onRemove: () => void; onClose: () => void
}) {
  const item = (danger = false, disabled = false): React.CSSProperties => ({
    width: '100%', display: 'flex', alignItems: 'center', gap: '10px',
    padding: '9px 10px', borderRadius: '8px', border: 'none', background: 'transparent',
    cursor: disabled ? 'not-allowed' : 'pointer', fontSize: '14px', textAlign: 'left',
    color: disabled ? 'rgba(255,255,255,0.2)' : danger ? '#FF5A5A' : 'var(--color-text-primary)',
  })
  return (
    <>
      ...
      <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, minWidth: '220px', background: 'var(--color-bg-elevated, #1a1a2e)', border: '1px solid rgba(151,125,255,0.2)', borderRadius: '14px', padding: '6px', zIndex: 99, boxShadow: '0 12px 32px rgba(0,0,0,0.5)' }}>
        {isInSuperset ? (
          <button onClick={() => { onDissolve(); onClose() }} style={{ ...item(), color: '#F5A623' }}>
            <SSIcon /> Superset auflösen
          </button>
        ) : (
          <button onClick={() => { if (canPair) { onStartPairing(); onClose() } }} disabled={!canPair} style={item(false, !canPair)}>
            <SSIcon /> Zum Superset hinzufügen
          </button>
        )}
        <div style={{ height: '1px', background: 'rgba(255,255,255,0.07)', margin: '6px 0' }} />
        <button onClick={() => { onRemove(); onClose() }} style={item(true)}>
          <TrashIcon /> Übung entfernen
        </button>
      </div>
    </>
  )
...
// ─── Main ──────────────────────────────────────────────────────────────────────
export default function WorkoutClient({ session, initialExercises, initialSets, existingPRs, previousSets }: Props) {
  ...
  // ── FIX: completedSets aus localStorage laden ──
  const [completedSets, setCompletedSets] = useState<Set<string>>(() => {
    try {
      const saved = localStorage.getItem(`completed_sets_${session.id}`)
  ...
  // ── FIX: completedSets in localStorage persistieren ──
  useEffect(() => {
    localStorage.setItem(`completed_sets_${session.id}`, JSON.stringify([...completedSets]))
  }, [completedSets, session.id])
  ...
  useEffect(() => {
    const hide = () => { (document.querySelector('nav') as HTMLElement | null)?.style.setProperty('display', 'none'); (document.querySelector('[data-workout-banner]') as HTMLElement | null)?.style.setProperty('display', 'none') }
    hide(); const t = setTimeout(hide, 500)
    return () => { clearTimeout(t); (document.querySelector('nav') as HTMLElement | null)?.style.setProperty('display', 'flex'); (document.querySelector('[data-workout-banner]') as HTMLElement | null)?.style.setProperty('display', 'flex') }
  }, [])
  useEffect(() => {
    if (initialExercises.length === 0) return
    loadEDBDataForExercises(initialExercises).then(updated => {
      setExercises(prev => prev.map(ex => {
        const withData = updated.find((u: any) => u.id === ex.id)
        return withData ? { ...ex, edbData: (withData as any).edbData } : ex
      }))
    })
  ...
  useEffect(() => {
    ...
    function onMove(e: TouchEvent) {
      e.preventDefault(); const cy = e.touches[0].clientY; const ds = dragStateRef.current; if (!ds) return
      const others = exercisesRef.current.filter(ex => ex.id !== ds.exerciseId); let ti = others.length
      for (let i = 0; i < others.length; i++) { const el = exerciseRefs.current.get(others[i].id); if (!el) continue; const r = el.getBoundingClientRect(); if (cy < r.top + r.height / 2) { ti = i; break } }
      setDragState(prev => prev ? { ...prev, currentY: cy, targetIndex: ti } : null)
    }
    function onEnd() {
      const ds = dragStateRef.current; if (!ds) return; const exs = exercisesRef.current
      if (ds.targetIndex !== ds.exerciseIndex) { const n = [...exs]; const [m] = n.splice(ds.exerciseIndex, 1); n.splice(ds.targetIndex, 0, m); const u = n.map((ex, i) => ({ ...ex, position: i })); setExercises(u); u.forEach(ex => supabase.from('workout_exercises').update({ position: ex.position }).eq('id', ex.id)) }
      setDragState(null)
    }
  ...
  async function addSet(weid: string) {
    const es = sets.filter(s => s.workout_exercise_id === weid); const ls = es[es.length - 1]
    const { data: ns } = await supabase.from('sets').insert({ workout_session_id: session.id, workout_exercise_id: weid, set_number: es.length + 1, weight_kg: ls?.weight_kg ?? null, reps: ls?.reps ?? null }).select().single()
    if (ns) setSets(p => [...p, ns])
  }
  async function removeSet(sid: string, weid: string) {
    await supabase.from('sets').delete().eq('id', sid)
    setSets(p => { const r = p.filter(s => s.id !== sid); let n = 1; return r.map(s => s.workout_exercise_id === weid ? { ...s, set_number: n++ } : s) })
  }
  async function updateSet(sid: string, field: string, value: string) {
    ...
    const us = sets.map(s => s.id === sid ? { ...s, [field]: nv } : s); setSets(us)
    const u = us.find(s => s.id === sid)
  ...
  function handleSetComplete(setId: string, exerciseId: string) {
    ...
    if (nextSet) {
      setHighlightSetId(nextSet.id)
      exerciseRefs.current.get(partner.id)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
      setTimeout(() => setHighlightSetId(null), 1800)
    }
  }
  ...
  async function removeExercise(weid: string) {
    ...
    if (ex?.superset_group) { await dissolveSuperset(ex.superset_group, 'workout_exercises'); setExercises(p => p.map(e => e.superset_group === ex.superset_group ? { ...e, superset_group: null } : e)) }
    await supabase.from('workout_exercises').delete().eq('id', weid)
    setSets(p => p.filter(s => s.workout_exercise_id !== weid)); setExercises(p => p.filter(e => e.id !== weid))
  }

  async function handleSupersetPair(targetId: string) {
    if (!supersetPickingFor) return
    const groupId = await pairSuperset(supersetPickingFor, targetId, 'workout_exercises')
  ...
  async function handleSupersetDissolve(groupId: string) {
    await dissolveSuperset(groupId, 'workout_exercises')
  ...
  async function saveTitle() {
    ...
    setTitle(trimmed)
    await supabase.from('workout_sessions').update({ title: trimmed }).eq('id', session.id)
  }

  function handleNameTouchStart(e: React.TouchEvent, exerciseId: string, index: number) {
    longPressFired.current = false; longPressStart.current = { y: e.touches[0].clientY }
    longPressTimer.current = setTimeout(() => { longPressFired.current = true; navigator.vibrate?.(40); setDragState({ exerciseId, exerciseIndex: index, startY: longPressStart.current.y, currentY: longPressStart.current.y, targetIndex: index }) }, 500)
  }
  ...
  async function finishWorkout() {
    const pi: any[] = []
    for (const [sid, pt] of Object.entries(newPRs)) {
      if (!pt.length) continue; const s = sets.find(x => x.id === sid); if (!s) continue; const ex = exercises.find(e => e.id === s.workout_exercise_id); if (!ex) continue
      for (const p of pt) { const v = p === 'max_weight' ? s.weight_kg : p === 'max_reps' ? s.reps : p === 'max_duration' ? s.duration_seconds : s.distance_km; if (v === null) continue; pi.push({ user_id: session.user_id, exercise_id: ex.exercise_id, workout_session_id: session.id, set_id: sid, pr_type: p, value: v }) }
    }
  ...
  async function deleteWorkout() {
    ...
    localStorage.removeItem(`completed_sets_${session.id}`)
    await deleteActiveWorkout(session.id); router.push('/routines'); router.refresh()
  }

  // ─── Exercise block renderer ────────────────────────────────────────────────
  function renderExBlock(ex: WorkoutExercise, index: number) {
    ...
    return (
      <div key={ex.id} ref={el => { if (el) exerciseRefs.current.set(ex.id, el); else exerciseRefs.current.delete(ex.id) }} style={{ opacity: isDragging ? 0.4 : 1, transition: 'opacity 0.15s' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-sm)', position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)', flex: 1, minWidth: 0 }}>
            <div
              ...
              style={{
                width: '36px', height: '36px', borderRadius: 'var(--radius-full)', overflow: 'hidden', flexShrink: 0,
                background: isPairingTarget ? 'color-mix(in srgb, var(--color-primary) 30%, transparent)' : 'color-mix(in srgb, var(--color-primary) 20%, transparent)',
                border: isPairingTarget ? '2px solid var(--color-primary)' : '1px solid color-mix(in srgb, var(--color-primary) 30%, transparent)',
                cursor: isPairingTarget ? 'pointer' : 'default', transition: 'all 0.2s',
            ...
            <button
              onClick={() => isPairingTarget ? handleSupersetPair(ex.id) : router.push(`/exercise/${ex.exercise_id}`)}
              onTouchStart={e => !supersetPickingFor && handleNameTouchStart(e, ex.id, index)}
              onTouchMove={handleNameTouchMove} onTouchEnd={handleNameTouchEnd}
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => !supersetPickingFor && setMenuOpenId(isMenuOpen ? null : ex.id)} style={{
              background: isMenuOpen ? 'rgba(151,125,255,0.12)' : 'none', border: 'none', borderRadius: '8px',
              color: 'var(--color-text-secondary)', fontSize: '18px', padding: '4px 8px', cursor: 'pointer',
              opacity: supersetPickingFor && supersetPickingFor !== ex.id ? 0.3 : 1, transition: 'all 0.15s',
            }}>···</button>
            {isMenuOpen && <ExerciseMenu isInSuperset={isInSS} canPair={canPair} onStartPairing={() => setSupersetPickingFor(ex.id)} onDissolve={() => ex.superset_group && handleSupersetDissolve(ex.superset_group)} onRemove={() => removeExercise(ex.id)} onClose={() => setMenuOpenId(null)} />}
          </div>
        </div>
        <div className="glass" style={{ overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '32px 1fr 1fr 1fr 36px', padding: '8px var(--spacing-md)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            {['Satz', 'Zuvor', 'kg', 'Wdh.', ''].map(h => <span key={h} style={{ fontSize: '11px', color: 'var(--color-text-secondary)', textAlign: 'center', fontWeight: '500' }}>{h}</span>)}
          </div>
          {exSets.map((s, si) => (
            <SwipeableSetRow key={s.id} s={s} i={si}
              isCompleted={completedSets.has(s.id)}
              hasPR={userEditedSets.has(s.id) && (newPRs[s.id]?.length ?? 0) > 0}
              isHighlighted={highlightSetId === s.id}
              prevValue={(() => {
                const prev = previousSets[ex.exercise_id]?.[si]
                if (!prev) return '—'
                if (prev.weight_kg != null && prev.reps != null) return `${prev.weight_kg}kg × ${prev.reps}`
                if (prev.weight_kg != null) return `${prev.weight_kg}kg`
                if (prev.reps != null) return `${prev.reps}`
                return '—'
              })()}
              onComplete={() => handleSetComplete(s.id, ex.id)}
              onUpdate={(f, v) => updateSet(s.id, f, v)}
              onDelete={() => removeSet(s.id, ex.id)}
            />
          ))}
          <button onClick={() => addSet(ex.id)} style={{ width: '100%', padding: 'var(--spacing-sm)', background: 'color-mix(in srgb, var(--color-primary) 10%, transparent)', border: 'none', color: 'var(--color-primary)', fontSize: 'var(--font-size-sm)', fontWeight: '500' }}>
            + Satz hinzufügen
          </button>
        </div>
      </div>
      ...
      <div className="glass-nav" style={{ position: 'sticky', top: supersetPickingFor ? '44px' : 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px var(--spacing-md)', zIndex: 100, borderTop: 'none', borderBottom: '1px solid color-mix(in srgb, var(--color-primary) 15%, transparent)', transition: 'top 0.2s' }}>
        ...
        {editingTitle ? (
          <input autoFocus value={titleDraft} onChange={e => setTitleDraft(e.target.value)} onBlur={saveTitle}
            ...
            style={{ background: 'transparent', border: 'none', borderBottom: '1px solid color-mix(in srgb, var(--color-primary) 60%, transparent)', color: 'var(--color-text-primary)', fontWeight: '600', fontSize: 'var(--font-size-md)', textAlign: 'center', width: '160px', outline: 'none', padding: '2px 4px' }}
          />
        ) : (
          <button onClick={() => { setTitleDraft(title); setEditingTitle(true) }}
            style={{ background: 'none', border: 'none', color: 'var(--color-text-primary)', fontWeight: '600', fontSize: 'var(--font-size-md)', cursor: 'text', padding: '4px 8px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '5px' }}>
            {title}
      ...
      <div style={{ padding: 'var(--spacing-md)', display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)' }}>
        ...
        {renderGroups.map((group) => {
          if (group.type === 'single') {
            ...
            return (
              <div key={group.ex.id}>
                {dragState && !isDragging && insertionLineAt === group.index && <div style={{ height: '2px', borderRadius: '2px', margin: '0 0 8px', background: 'var(--color-primary)', boxShadow: '0 0 6px color-mix(in srgb, var(--color-primary) 60%, transparent)' }} />}
                {renderExBlock(group.ex, group.index)}
              </div>
            )
          }
          return (
